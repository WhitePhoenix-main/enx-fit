...
(function($) {
  ...
  function isLocalRequestUrl(value) {
  ...
  var Unifato = {
      ...
      var defaults = {
        ...
        onLabelShow: function(event, label, code) {
          var mapValues = $(this)[0].mapValues,
            $this = $(this),
            place = label[0].textContent;
          if( mapValues !== undefined) {
            var value = $(this)[0].mapValues[code];
            if(value!==undefined ) {
              value = $(this)[0].mapValues[code];
              if($this.data('plugin-options').label !== undefined) {
                label[0].textContent = $this.data('plugin-options').label.replace('%p', place).replace('%v', value);
              }
              else
                label[0].textContent = label[0].textContent + ":  " + value;
            }
            else if(value===undefined) {
              if($this.data('plugin-options').label !== undefined) {
                label[0].textContent = $this.data('plugin-options').labelNaN.replace('%p', place);
              }
              else
                label[0].textContent = 'NaN';
            }
          }
          else if( $this.data('plugin-options').label !== undefined ) {
            label[0].textContent = $this.data('plugin-options').label.replace('%p',place);
          }
        },
      };
      el.each(function(index) {
        ...
        if(typeof(options.valuesSrcFile) !== 'undefined' && isLocalRequestUrl(options.valuesSrcFile)) {
          $.ajax({
    ...
    enableFullCalendar: function() {
      ...
      el.each(function(index){
        ...
        if(calendarOptions.eventsSrc!==undefined && isLocalRequestUrl(calendarOptions.eventsSrc)) {
          $.ajax({
    ...
    enableTimelineLoadMoreBtn: function() {
      ...
      el.each(function(){
        ...
        $this.on('click', function() {
          if (!isLocalRequestUrl(url)) return;
